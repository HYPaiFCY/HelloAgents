...
class ExpandableToolTemplate(Tool):
    ...
    def create(self, name: str, content: str, tags: str = "") -> ToolResponse:
        ...
        return ToolResponse.success(
            text=f"资源 '{name}' 创建成功", data={"resource": resource}
        )
    ...
    def delete(self, name: str, confirm: bool = False) -> ToolResponse:
        ...
        return ToolResponse.success(
            text=f"资源 '{name}' 已删除", data={"deleted_resource": deleted_resource}
        )
...
if __name__ == "__main__":
    ...
    response = registry.execute_tool(
        "expandable_create",
        {"name": "doc1", "content": "这是第一个文档", "tags": "重要, 文档"},
    )
    ...
    # # 读取资源
    # response = registry.execute_tool(
    #     "expandable_read", {"name": "doc1", "include_metadata": True}
    # )
    # print(f"读取: {response.text}")

    # # 更新资源
    # response = registry.execute_tool(
    #     "expandable_update", {"name": "doc1", "content": "更新后的内容"}
    # )
    ...
    # # 删除资源
    # response = registry.execute_tool(
    #     "expandable_delete", {"name": "doc1", "confirm": True}
    # )
    # print(f"删除: {response.text}")

    # for name, tool in registry._tools.items():
    #     print(f"工具名称: {name}, 描述: {tool.description}")
    #     print(f"  参数: {[param.name for param in tool.get_parameters()]}")
