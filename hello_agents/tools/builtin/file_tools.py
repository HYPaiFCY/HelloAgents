...
def _resolve_workspace_path(project_root: Path, working_dir: Path, path: str) -> Path:
    """解析并限制路径在 project_root 内，防止绝对路径和 .. 路径逃逸。"""
    normalized = path.replace("\\", "/")
    candidate = Path(normalized)
    full_path = candidate if candidate.is_absolute() else working_dir / candidate
    full_path = full_path.resolve()

    if not _is_relative_to(full_path, project_root):
        raise PermissionError(f"路径 '{path}' 超出项目根目录 '{project_root}'")
...
class ReadTool(Tool):
    ...
    def _resolve_path(self, path: str) -> Path:
        """解析路径，并限制在项目根目录内。"""
        return _resolve_workspace_path(self.project_root, self.working_dir, path)
...
class WriteTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        if content is None:
            return ToolResponse.error(
                code=ToolErrorCode.INVALID_PARAM, message="缺少必需参数: content"
            )
...
class EditTool(Tool):
    ...
    def run(self, parameters: Dict[str, Any]) -> ToolResponse:
        ...
        if old_string is None:
            return ToolResponse.error(
                code=ToolErrorCode.INVALID_PARAM, message="缺少必需参数: old_string"
            )

        if new_string is None:
            return ToolResponse.error(
                code=ToolErrorCode.INVALID_PARAM, message="缺少必需参数: new_string"
            )
