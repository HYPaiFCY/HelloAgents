...
def load_and_chunk_texts(paths: List[str], chunk_size: int = 800, chunk_overlap: int = 100, namespace: Optional[str] = None, source_label: str = "rag") -> List[Dict]:
    ...
    for path in paths:
        ...
        # 对于所有格式都使用markdown-aware处理，保持一致性和结构信息
        ...
        for ch in token_chunks:
            ...
            # content_hash（用于去重/指纹）和 chunk_id（用于唯一标识与索引） 
...
def build_graph_from_chunks(neo4j, chunks: List[Dict]) -> None:
    """基于块的元数据构建知识图谱，关联文档和块。

    改进点：
    - 使用 `logging` 记录异常信息（而不是静默忽略），便于排查。
    - 收集失败项并返回，调用方可据此重试或报警。
    - 保持对已创建文档的去重。

    返回值：失败项列表（若全部成功返回空列表）。
    """
    ...
    for ch in chunks:
        ...
        if doc_id and doc_id not in created_docs:
            ...
            try:
                ...
                logger.exception("Failed to create Document node %s: %s", doc_id, e)
                failures.append({"action": "add_entity_doc", "doc_id": doc_id, "error": str(e), "chunk": ch})
        ...
        try:
            ...
            logger.exception("Failed to create Memory node %s: %s", mem_id, e)
            failures.append({"action": "add_entity_mem", "mem_id": mem_id, "error": str(e), "chunk": ch})

        if doc_id:
            try:
                neo4j.add_relationship(from_id=doc_id, to_id=mem_id, rel_type="HAS_CHUNK", properties={})
            except Exception as e:
                logger.exception("Failed to create relationship %s -> %s: %s", doc_id, mem_id, e)
                failures.append({"action": "add_relationship", "doc_id": doc_id, "mem_id": mem_id, "error": str(e), "chunk": ch})

    if failures:
        logger.warning("build_graph_from_chunks completed with %d failures", len(failures))
    else:
        logger.info("build_graph_from_chunks completed successfully: processed %d chunks", len(chunks or []))
...
def _preprocess_markdown_for_embedding(text: str) -> str:
    ...
    """
    """ 对 Markdown 文本进行预处理，以提升嵌入质量。
    在保留语义内容的同时，移除多余的标记。"""
    import re
    
    # Remove markdown headers symbols but keep the text
    # 注意：保留标题文本内容，但移除#符号，避免对嵌入造成干扰
    text = re.sub(r'^#{1,6}\s+', '', text, flags=re.MULTILINE)
    
    # Remove markdown links but keep the text
    # 注意：保留链接文本，但移除URL，避免对嵌入造成干扰
    text = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', text)
    
    # Remove markdown emphasis markers
    # 注意：保留强调文本，但移除强调标记，避免对嵌入造成干扰
    text = re.sub(r'\*\*([^*]+)\*\*', r'\1', text)  # bold
    text = re.sub(r'\*([^*]+)\*', r'\1', text)      # italic
    text = re.sub(r'`([^`]+)`', r'\1', text)        # inline code
    
    # Remove markdown code blocks but keep content
    # 注意：保留代码块内容，但移除```标记，避免对嵌入造成干扰
    text = re.sub(r'```[^\n]*\n([\s\S]*?)```', r'\1', text)
    
    # Remove excessive whitespace
    # 注意：保留合理的段落分隔，但移除多余的空行和空格，提升文本质量
    text = re.sub(r'\n\s*\n', '\n\n', text)
    text = re.sub(r'[ \t]+', ' ', text)
...
def index_chunks(
    ...
    """
    """使用统一嵌入和Qdrant存储对Markdown片段进行索引。"""
    if not chunks:
...
class AdvancedRAGPipeline:
    ...
    该类不替换原有 `create_rag_pipeline`，而是在已有函数基础上串联完整检索流程：
    文档加载/分块 -> 向量索引 -> 扩展召回 -> CrossEncoder 重排 -> 图信号融合 -> 邻居扩展 -> 压缩 -> 引用合并。
    """
    ...
    def add_documents(
        ...
        if build_graph and self.neo4j is not None and chunks:
            graph_failures = build_graph_from_chunks(self.neo4j, chunks)

        return {
            "input_files": len(file_paths or []),
            "chunks": len(chunks),
            "namespace": self.namespace,
            "graph_enabled": bool(build_graph and self.neo4j is not None),
    ...
    def retrieve(
        ...
        neighbors: int = 1,
        max_neighbor_additions: int = 5,
        compress: bool = True,
        max_per_doc: int = 2,
        join_gap: int = 200,
    ) -> List[Dict]:
        """返回结构化检索结果，适合继续给 LLM 或 UI 使用。"""
        if not query:
            return []

        pool_top_k = max(top_k, top_k * max(1, candidate_pool_multiplier))
        pool = search_vectors_expanded(
            store=self.store,
            query=query,
            top_k=pool_top_k,
            rag_namespace=self.namespace,
        ...
        candidates = pool
        if rerank:
            candidates = rerank_with_cross_encoder(
                query=query,
                items=list(pool),
                model_name=rerank_model,
                top_k=min(len(pool), pool_top_k),
            )

        graph_signals: Dict[str, float] = {}
        if use_graph_signals:
            graph_signals = compute_graph_signals_from_pool(
                candidates,
        ...
        ranked = rank(
            ...
            w_graph=graph_weight if use_graph_signals else 0.0,
        )
        selected = ranked[:top_k]

        if expand_neighbors:
            selected = expand_neighbors_from_pool(
                selected=selected,
                pool=ranked,
                neighbors=neighbors,
                max_additions=max_neighbor_additions,
            )

        selected = compress_ranked_items(
            selected,
        ...
        return selected[: max(top_k, len(selected))]
    ...
    def answer_context(
        ...
        **kwargs: Any,
        ...
        """执行高级检索并返回可直接注入 LLM Prompt 的上下文。"""
        items = self.retrieve(query=query, top_k=top_k, **kwargs)
        context = merge_snippets_grouped(
            items,
            max_chars=max_chars,
            include_citations=include_citations,
        )
        summary = tldr_summarize(context) if summarize and context else None
        return {
            "query": query,
            "namespace": self.namespace,
            "items": items,
    ...
    def search(self, query: str, top_k: int = 8, **kwargs: Any) -> List[Dict]:
        """高级检索的简短别名。"""
